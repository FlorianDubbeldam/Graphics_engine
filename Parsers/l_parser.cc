...
namespace
{
	...
	void parse_rules(std::set<char> const& alphabet, std::map<char, std::map<std::string, double>>& rules, stream_parser& parser, bool parse2D)
	{
		...
		while (true)
		{
            ...
            // Nu extra stukje voor die stochastische dingen

            bool stochastic = false;

            //char accolade = parser.getChar();

            std::string rule;
            try{
                rule = parser.readQuotedString();
            }catch (...){
                //std::cout << "stochastische regels gevonden!" << std::endl;
                stochastic = true;
            }

            // Heeft stochastische regels
            if (stochastic){

                while(true){
                    parser.skip_comments_and_whitespace();
                    //De kans moet altijd in een string staan!, pech dan.
                    // Trouwens, er is schijnbaar ook een parser.readDouble, misschien later nog doen
                    ...
                    // nu neem je de echte rule

                    std::string rule = parser.readQuotedString();

                    // Effe checken of alles goed is(gekopieerd, ni zo mooi)
                    ...
                    rules[alphabet_char][rule] = chance;
...
std::string const& LParser::LSystem::get_replacement(char c) const
{
    ...
    //GEZELLIG AANPASSEN JONGENTJES
    //Ik heb er ook maar meteen de kans enzo uitgerekend(ik ben er eigenlijk niet zo voor, maar misschien moet
    // zo'n oud programma ook op dit systeem kunnen runnen, dus dan moet ik de aanpassingen wel hier maken,
    // want ik mag de functie-calls niet veranderen

    double chance = replacementrules.find(c)->second.begin()->second; //.find(1.0)->second;
    //zonder static verwijdert hij de string nadat hij hem teruggeeft :(
    static std::string returnReplacementrule;

    if (chance == 1.0){
        returnReplacementrule = replacementrules.find(c)->second.begin()->first;
    }
    //Stochastische shit
    else{
        std::vector<std::string> rules;
        std::vector<double> probabilities;
        // Was vergeten dat dictionary maar 1 searchkey kan hebben, dus wisselen double en string om :)


        for (auto& pair : replacementrules.find(c)->second){
        ...
        // Hij kiest met die verschillende waarschijnlijkheden, 1 van die waarschijnlijkheden omdat die
        // de index zijn van de map. Beetje verwarrend, kweet :)
        // Ik denk dat je nu helemaal in de war bent, dat is het waarschijnlijkst \(`0`)/
        std::string random_rule = rules[random_index];

        //std::cout << random_value << std::endl;

        returnReplacementrule = random_rule;//replacementrules.find(c)->second.find(random_rule)->second;
